...
Log::Log() {
    ...
    // 创建线程异步写入文件
    std::thread write_log_thread([this] { async_write_log(); });
...
void Log::write_log(int level, int line, const char* file, const char *format, ...) {
    ...
    // 格式化输出内容
    {
        std::lock_guard<std::mutex> locker(buffer_mutex_);    
        // 写入缓冲区
        ...
        int mn = vsnprintf(buffer_ + m + n, LOG_BUFFER_SIZE - 1, format, valist);
        buffer_[n + m + mn] = '\n';
        buffer_[n + m + mn + 1] = '\0';
